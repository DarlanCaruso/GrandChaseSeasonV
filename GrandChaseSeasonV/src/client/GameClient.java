...
public class GameClient extends Client {
	...
	public void onPacket(Packet p) {
		...
		switch( Opcode ) {
		case OpcodeGame.HEART_BIT_NOT: // 하트비트 (시즌5)
			heartbit = System.currentTimeMillis();
			break;
		case OpcodeGame.EVENT_VERIFY_ACCOUNT_REQ: // 로그인 (시즌5)
			getGameUser().EVENT_VERIFY_ACCOUNT_REQ(p);
			break;
		case OpcodeGame.EVENT_CHAR_SELECT_JOIN_REQ: // 캐릭터 접속 (시즌5)
		...
		case OpcodeGame.EVENT_CHANGE_ROOMUSER_INFO_REQ: // 방 유저정보 바뀜
			...
			break;
		case OpcodeGame.EVENT_EQUIP_ITEM_REQ: // 장비 장착 변경 요청
			Main.printmsg("테스트 (OP:" + Opcode + ", SIZE: " + dataSize + "byte)\n" + Convert.byteArrayToHexString(p.buffer));
		...
		case OpcodeGame.EVENT_SQUARE_USER_SYNC_REQ: // 광장 싱크 요청
			...
			break;
		case OpcodeGame.EVENT_CHANGE_CHARACTER_INFO_REQ: // 캐릭터 선택 변경 (시즌5)
			getGameUser().EVENT_CHANGE_CHARACTER_INFO_REQ(p);
			break;
		default:
